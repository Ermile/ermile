...
/**
 * attachment addons v0.0.1
 */

attachment_addons = (function(){
  var ajax, time_start;
  var options = {}
  var back_change = '';
  ...
  var get_url = function()
  {
    ...
    url_match = url.match(/^\/cp\/([^\/]+).*$/);
    if(url_match)
    {
      return url_match[1] + "/attachments_data/";
    }
  }

  var start = function(_url)
  {
    var url = get_url() + _url;
    $(window).on('statechange', function(_event){
      console.log(_event);
  ...
  $("[data-ejax-group=attachment-addons]").bind('keyup click', function(_event){
    ...
    back_change = $("[name=attachments_search_q]").val();
    if(time_start)
    {
      clearTimeout(time_start);
    }
    var url = Array();
    $("[data-ejax-group=attachment-addons]").each(function(){
      if($(this).val() != '')
      {
